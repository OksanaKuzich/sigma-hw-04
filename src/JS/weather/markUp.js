...
export function markUpCard(city) {
  return `<div class="container__weather">
          <h2 class="city__name">Weather in ${city.name}, ${
    city.sys.country
  }</h2>
          <p class="current__date">${format(new Date(), 'd MMMM, eeee')}</p>
          <div class="wrapper">
          <div class="weather">
          ...
          <p class="type__weather">${city.weather[0].description}</p>
          </div>
          <div class="info">
          ...
          <button class="btn__more" type="button" data-lat="${
            city.coord.lat
...
export function markUpList(items) {
  return items
    .map(({ name, state, country, lat, lon }) => {
      if (state === undefined) {
        return `<li class='list'><button class="btn__list" type="button" data-lat="${lat}" data-lon="${lon}">${name}, ${country}</button></li>`;
      }
      return `<li class='list'><button class="btn__list" type="button" data-lat="${lat}" data-lon="${lon}">${name}, ${state}, ${country}</button></li>`;
    })
    .join('');
}
